    ...
    try {
        ...
        if (data.status === '2fa_required') {
            ...
            document.getElementById('otp-submit').onclick = async function() {
                ...
                otpFormData.append('username', formData.get('username'));
                otpFormData.append('otp', otp);

                try {
                    const otpData = await myFetch('https://localhost:8443/user_auth/login/', otpFormData, 'POST', false);

                    if (otpData.access && otpData.refresh) {
                        console.log('Login successful');
                        localStorage.setItem('access_token', otpData.access);
                        localStorage.setItem('refresh_token', otpData.refresh);
